...
## P2-B08 implementation record

P2-B08 implements temporary privilege, support access and break-glass only as bounded exceptional authorization
sources. The decision does not revise P0/P1 boundaries: MySQL/InnoDB remains authoritative, B06 roles remain the only
administrative authority, B07 Tenant Context remains session-owned, and exceptional activation cannot create an
ordinary role, membership, account context or impersonation. Its immutable policy snapshot, independent support
approvals, phishing-resistant break-glass step-up, session binding, expiry/revocation, mandatory support/emergency
review and no-restoration rule are controlled implementation decisions recorded in
`privileged-access-temporary-support-and-break-glass-standard.md`.
