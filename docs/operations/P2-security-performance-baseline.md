...
The in-memory three-loop performance test completed in 0.172 seconds during B10 validation. This is a local
regression observation, not a database or production service target. The isolated full MySQL P2 suite completed in
14 minutes 39 seconds, including schema reset, migrations, seeds and concurrency tests; it is not used as an SLO.
