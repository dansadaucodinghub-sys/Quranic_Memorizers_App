...
if (
    !is_array($payload)
    || !is_int($payload['workspace_id'] ?? null)
    || !is_string($payload['organization_id'] ?? null)
    || !is_int($payload['expected_version'] ?? null)
) {
    throw new RuntimeException('Organization mutation worker payload is invalid.');
}
...
$statement = $connection->prepare(
    "UPDATE organizations SET version = version + 1, updated_at = UTC_TIMESTAMP(6) "
    . "WHERE workspace_id = :workspace_id AND public_id = UUID_TO_BIN(:organization_id) "
