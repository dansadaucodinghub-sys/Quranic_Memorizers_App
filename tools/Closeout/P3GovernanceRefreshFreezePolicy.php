...
final class P3GovernanceRefreshFreezePolicy
{
    public const MANIFEST = 'docs/closeout/p3/qmdb-p3-governance-extension-freeze-002.yaml';
    ...
    public function entries(string $root): array
    {
        ...
        foreach (FileSystem::files($root) as $absolute) {
            ...
            if (is_link($absolute) || (new PathPolicy())->repositoryViolation($path) !== null) {
                throw new \RuntimeException('P3 governance refresh rejects governed path: ' . $path);
            }
            $hash = hash_file('sha256', $absolute);
            if (!is_string($hash)) {
                throw new \RuntimeException('Unable to hash P3 governance refresh file: ' . $path);
            }
