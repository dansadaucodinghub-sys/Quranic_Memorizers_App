...
CREATE TABLE organization_units (
 ...
 PRIMARY KEY (id), UNIQUE KEY uq_organization_units_public_id (public_id), UNIQUE KEY uq_organization_units_registry_code (registry_code), UNIQUE KEY uq_organization_units_workspace_organization_id (workspace_id,organization_id,id), UNIQUE KEY uq_organization_units_active_primary (workspace_id,organization_id,active_primary_marker), KEY ix_organization_units_organization_status (workspace_id,organization_id,status,depth,id), KEY ix_organization_units_parent_status (workspace_id,organization_id,parent_unit_id,status,id), CONSTRAINT fk_organization_units_organization FOREIGN KEY (workspace_id,organization_id) REFERENCES organizations (workspace_id,id) ON DELETE RESTRICT ON UPDATE RESTRICT, CONSTRAINT fk_organization_units_parent FOREIGN KEY (workspace_id,organization_id,parent_unit_id) REFERENCES organization_units (workspace_id,organization_id,id) ON DELETE RESTRICT ON UPDATE RESTRICT, CONSTRAINT fk_organization_units_creator FOREIGN KEY (created_by_account_id) REFERENCES user_accounts (id) ON DELETE RESTRICT ON UPDATE RESTRICT, CONSTRAINT ck_organization_units_type CHECK (unit_type IN ('HEADQUARTERS','BRANCH','CAMPUS','CENTRE','SCHOOL','MOSQUE','OFFICE','OTHER')), CONSTRAINT ck_organization_units_status CHECK (status IN ('ACTIVE','RETIRED')), CONSTRAINT ck_organization_units_depth CHECK (depth <= 8), CONSTRAINT ck_organization_units_root_shape CHECK ((parent_unit_id IS NULL AND is_primary=1 AND depth=0) OR (parent_unit_id IS NOT NULL AND is_primary=0 AND depth>=1)), CONSTRAINT ck_organization_units_version CHECK (version >= 1), CONSTRAINT ck_organization_units_retired CHECK (status <> 'RETIRED' OR retired_at IS NOT NULL)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci
SQL),
        new SqlMigrationStep(new MigrationStepId('002_create_organization_unit_names'), 'Create Unit private name history.', <<<'SQL'
CREATE TABLE organization_unit_names (
 id BIGINT UNSIGNED NOT NULL AUTO_INCREMENT, public_id BINARY(16) NOT NULL, workspace_id BIGINT UNSIGNED NOT NULL, organization_id BIGINT UNSIGNED NOT NULL, unit_id BIGINT UNSIGNED NOT NULL, name_type VARCHAR(16) CHARACTER SET ascii COLLATE ascii_bin NOT NULL, script_code VARCHAR(16) CHARACTER SET ascii COLLATE ascii_bin NOT NULL, display_name VARCHAR(240) NOT NULL, search_name VARCHAR(320) NOT NULL, status VARCHAR(16) CHARACTER SET ascii COLLATE ascii_bin NOT NULL, version INT UNSIGNED NOT NULL DEFAULT 1, effective_at DATETIME(6) NOT NULL, superseded_at DATETIME(6) NULL, created_at DATETIME(6) NOT NULL, updated_at DATETIME(6) NOT NULL, active_name_marker TINYINT UNSIGNED GENERATED ALWAYS AS (CASE WHEN status='ACTIVE' THEN 1 ELSE NULL END) STORED,
 PRIMARY KEY (id), UNIQUE KEY uq_organization_unit_names_public_id (public_id), UNIQUE KEY uq_organization_unit_names_active_type (workspace_id,organization_id,unit_id,name_type,active_name_marker), KEY ix_organization_unit_names_search (workspace_id,organization_id,search_name,status,id), CONSTRAINT fk_organization_unit_names_unit FOREIGN KEY (workspace_id,organization_id,unit_id) REFERENCES organization_units (workspace_id,organization_id,id) ON DELETE RESTRICT ON UPDATE RESTRICT, CONSTRAINT ck_organization_unit_names_type CHECK (name_type IN ('PRIMARY','SHORT','ARABIC','ALIAS')), CONSTRAINT ck_organization_unit_names_script CHECK (script_code IN ('LATIN','ARABIC','OTHER')), CONSTRAINT ck_organization_unit_names_status CHECK (status IN ('ACTIVE','SUPERSEDED')), CONSTRAINT ck_organization_unit_names_version CHECK (version >= 1), CONSTRAINT ck_organization_unit_names_superseded CHECK (status <> 'SUPERSEDED' OR superseded_at IS NOT NULL)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci
SQL),
        new SqlMigrationStep(new MigrationStepId('003_create_organization_unit_locations'), 'Create Unit self-declared location history.', <<<'SQL'
CREATE TABLE organization_unit_locations (
 id BIGINT UNSIGNED NOT NULL AUTO_INCREMENT, public_id BINARY(16) NOT NULL, workspace_id BIGINT UNSIGNED NOT NULL, organization_id BIGINT UNSIGNED NOT NULL, unit_id BIGINT UNSIGNED NOT NULL, country_id BIGINT UNSIGNED NOT NULL, level_one_area_id BIGINT UNSIGNED NULL, level_two_area_id BIGINT UNSIGNED NULL, source_type VARCHAR(24) CHARACTER SET ascii COLLATE ascii_bin NOT NULL, status VARCHAR(16) CHARACTER SET ascii COLLATE ascii_bin NOT NULL, version INT UNSIGNED NOT NULL DEFAULT 1, effective_at DATETIME(6) NOT NULL, superseded_at DATETIME(6) NULL, created_at DATETIME(6) NOT NULL, updated_at DATETIME(6) NOT NULL, active_marker TINYINT UNSIGNED GENERATED ALWAYS AS (CASE WHEN status='ACTIVE' THEN 1 ELSE NULL END) STORED,
