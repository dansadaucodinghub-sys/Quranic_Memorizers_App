...
final readonly class MySqlOrganizationRegistryRepository implements OrganizationRegistryRepository
{
    ...
    public function organization(TenantContext $tenant, string $publicId, bool $forUpdate = false): ?array
    {
        ...
        $record['classification_codes'] = $this->classificationCodes($tenant, (int) $record['id']);
        return $record;
    }
    ...
    public function create(TenantContext $tenant, int $accountId, string $publicId, string $code, array $input, DateTimeImmutable $now): array
    {
        ...
        $this->insertName('organization_names', $tenant->workspaceInternalId(), $organizationId, null, (string) $input['primary_name'], (string) $input['primary_script']);
        $this->insertClassifications($tenant, $organizationId, $input);
        $this->insertJurisdiction($tenant, $organizationId, $input);
        $unitId = $this->insertUnit($tenant, $organizationId, null, $accountId, (string) $input['unit_name'], (string) $input['unit_script'], (string) $input['unit_type'], true, 0);
    ...
    {
        unset($now); $this->assertVersion($organization, $expectedVersion);
        ...
        $statement->execute(['organization_id' => $organization['id'], 'workspace_id' => $tenant->workspaceInternalId(), 'version' => $expectedVersion]);
        if ($statement->rowCount() !== 1) throw new \DomainException('Organization is stale or unavailable.');
        $organizationId = (int) $organization['id'];
        $this->supersedeAndInsertName('organization_names', $tenant->workspaceInternalId(), $organizationId, null, (string) $input['primary_name'], (string) $input['primary_script']);
        $this->replaceClassifications($tenant, $organizationId, $input);
        $this->replaceJurisdiction($tenant, $organizationId, $input);
        return $this->required($this->organization($tenant, UuidV7::fromBinary((string) $organization['public_id'])->toString(), true));
    }

    public function retire(TenantContext $tenant, array $organization, int $expectedVersion, DateTimeImmutable $now): int
    {
        unset($now); $this->assertVersion($organization, $expectedVersion); $db = $this->db();
        ...
        $units->execute(['workspace_id' => $tenant->workspaceInternalId(), 'organization_id' => $organization['id']]);
    ...
    public function units(TenantContext $tenant, array $organization): array
    {
        ...
        $statement->execute(['workspace_id' => $tenant->workspaceInternalId(), 'organization_id' => $organization['id']]);
        return $statement->fetchAll(PDO::FETCH_ASSOC) ?: [];
    }
    ...
    public function unit(TenantContext $tenant, array $organization, string $publicId, bool $forUpdate = false): ?array
    {
        ...
        $statement = $this->db()->prepare($sql);
        $statement->execute(['workspace_id' => $tenant->workspaceInternalId(), 'organization_id' => $organization['id'], 'public_id' => $publicId]);
        $record = $statement->fetch(PDO::FETCH_ASSOC);
    ...
    {
        unset($now);
        if ($parent === null) throw new \InvalidArgumentException('Organization Unit parent is required.');
        if ($parent['status'] !== 'ACTIVE') throw new \DomainException('Organization Unit parent is unavailable.');
        $depth = (int) $parent['depth'] + 1;
        if ($depth > $this->configuration->unitMaximumDepth) throw new \DomainException('Organization Unit maximum depth has been reached.');
        ...
        $count->execute(['workspace_id' => $tenant->workspaceInternalId(), 'organization_id' => $organization['id']]);
        if ((int) $count->fetchColumn() >= $this->configuration->maximumUnits) throw new \DomainException('Organization Unit capacity has been reached.');
        $unitId = $this->insertUnit($tenant, (int) $organization['id'], (int) $parent['id'], $accountId, (string) $input['unit_name'], (string) $input['unit_script'], (string) $input['unit_type'], false, $depth, $publicId, $code);
        $this->insertLocation($tenant, (int) $organization['id'], $unitId, $input);
    ...
    {
        unset($now); $this->assertVersion($unit, $expectedVersion);
        ...
        $statement->execute(['unit_type' => $input['unit_type'], 'unit_id' => $unit['id'], 'workspace_id' => $tenant->workspaceInternalId(), 'organization_id' => $organization['id'], 'version' => $expectedVersion]);
        if ($statement->rowCount() !== 1) throw new \DomainException('Organization Unit is stale or unavailable.');
        $this->supersedeAndInsertName('organization_unit_names', $tenant->workspaceInternalId(), (int) $organization['id'], (int) $unit['id'], (string) $input['unit_name'], (string) $input['unit_script']);
        $this->replaceLocation($tenant, (int) $organization['id'], (int) $unit['id'], $input);
        return $this->required($this->unit($tenant, $organization, UuidV7::fromBinary((string) $unit['public_id'])->toString(), true));
    }
    ...
    {
        unset($now); $this->assertVersion($unit, $expectedVersion);
        if ((int) $unit['depth'] === 0) throw new \DomainException('The primary Unit cannot be retired independently.');
        ...
        $children->execute(['workspace_id' => $tenant->workspaceInternalId(), 'organization_id' => $organization['id'], 'unit_id' => $unit['id']]);
        if ((int) $children->fetchColumn() > 0) throw new \DomainException('Organization Unit has active children.');
        ...
        $statement->execute(['unit_id' => $unit['id'], 'workspace_id' => $tenant->workspaceInternalId(), 'organization_id' => $organization['id'], 'version' => $expectedVersion]);
        if ($statement->rowCount() !== 1) throw new \DomainException('Organization Unit is stale or unavailable.');
    }
    ...
    public function report(): array
    {
        ...
        $units = (int) $db->query('SELECT COUNT(*) FROM organization_units')->fetchColumn();
        $invalid = (int) $db->query("SELECT (SELECT COUNT(*) FROM organizations o WHERE NOT EXISTS(SELECT 1 FROM organization_names n WHERE n.workspace_id=o.workspace_id AND n.organization_id=o.id AND n.name_type='PRIMARY' AND n.status='ACTIVE'))+(SELECT COUNT(*) FROM organizations o WHERE NOT EXISTS(SELECT 1 FROM organization_classification_assignments a WHERE a.workspace_id=o.workspace_id AND a.organization_id=o.id AND a.status='ACTIVE' AND a.is_primary=1))")->fetchColumn();
    ...
    {
        $publicId ??= UuidV7::generate()->toString(); $code ??= 'QMU-' . bin2hex(random_bytes(10));
    ...
    private function insertName(string $table, int $workspaceId, int $organizationId, ?int $unitId, string $name, string $script): void
    {
        ...
        if ($unitId !== null) $parameters['unit_id'] = $unitId;
    ...
    private function supersedeAndInsertName(string $table, int $workspaceId, int $organizationId, ?int $unitId, string $name, string $script): void
    {
        ...
        $parameters = ['workspace_id' => $workspaceId, 'organization_id' => $organizationId]; if ($unitId !== null) $parameters['unit_id'] = $unitId;
        $statement->execute($parameters);
        if ($statement->rowCount() !== 1) throw new \DomainException('Current Organization name is unavailable.');
    ...
    {
        foreach ($input['classification_codes'] as $code) {
            ...
            $statement->execute(['public_id' => UuidV7::generate()->toString(), 'workspace_id' => $tenant->workspaceInternalId(), 'organization_id' => $organizationId, 'is_primary' => $code === $input['primary_classification'] ? 1 : 0, 'code' => $code]);
            if ($statement->rowCount() !== 1) throw new \InvalidArgumentException('Organization classification is unavailable.');
        }
    }
    ...
    private function insertJurisdiction(TenantContext $tenant, int $organizationId, array $input): void
    {
        ...
        $statement->execute(['public_id' => UuidV7::generate()->toString(), 'workspace_id' => $tenant->workspaceInternalId(), 'organization_id' => $organizationId, 'jurisdiction_level' => $input['jurisdiction_level'], 'country_public_id' => $input['country_public_id'], 'level_one_public_id' => $input['level_one_public_id'], 'level_two_public_id' => $input['level_two_public_id']]);
    }
    ...
    {
        if ($input['unit_country_public_id'] === null) return;
        ...
        $statement->execute(['public_id' => UuidV7::generate()->toString(), 'workspace_id' => $tenant->workspaceInternalId(), 'organization_id' => $organizationId, 'unit_id' => $unitId, 'country_public_id' => $input['unit_country_public_id'], 'level_one_public_id' => $input['unit_level_one_public_id'], 'level_two_public_id' => $input['unit_level_two_public_id']]);
    }
    ...
    private function classificationCodes(TenantContext $tenant, int $organizationId): array
    {
        ...
        $statement->execute(['workspace_id' => $tenant->workspaceInternalId(), 'organization_id' => $organizationId]);
        return array_map(static fn (array $row): string => $row['code'], $statement->fetchAll(PDO::FETCH_ASSOC) ?: []);
    }

    private function db(): PDO { return $this->provider->connection(); }
    private function search(string $name): string { return mb_strtolower(preg_replace('/\s+/u', ' ', trim($name)) ?? '', 'UTF-8'); }
    private function required(?array $record): array { if ($record === null) throw new \UnexpectedValueException('Organization persistence result is unavailable.'); return $record; }
    private function assertVersion(array $record, int $expectedVersion): void { if ((int) ($record['version'] ?? 0) !== $expectedVersion) throw new \DomainException('Organization record is stale.'); }
}
