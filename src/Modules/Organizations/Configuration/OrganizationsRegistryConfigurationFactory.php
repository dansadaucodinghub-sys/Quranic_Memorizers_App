...
final readonly class OrganizationsRegistryConfigurationFactory
{
    ...
    private function integer(EnvironmentVariables $v,string $name,int $default,int $min,int $max): int { $value=$v->optionalString($name); if($value===null){return $default;} if(preg_match('/\A[1-9][0-9]*\z/',$value)!==1||(int)$value<$min||(int)$value>$max){throw new \InvalidArgumentException($name.' is outside its approved range.');} return (int)$value; }
}
