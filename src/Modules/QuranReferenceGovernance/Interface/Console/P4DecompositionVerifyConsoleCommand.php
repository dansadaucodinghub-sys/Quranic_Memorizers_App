...
final readonly class P4DecompositionVerifyConsoleCommand implements ConsoleCommand
{
    ...
    public function name(): ConsoleCommandName { return new ConsoleCommandName('governance:p4-decomposition:verify'); }
    public function description(): string { return 'Verify the owner-approved P4 decomposition and authorization boundary.'; }
    public function execute(ConsoleInput $input, ConsoleOutput $output): int
    {
        ...
        $valid = is_string($decomposition) && is_string($authorization);
        foreach ($required as $value) $valid = $valid && str_contains($decomposition, $value);
        foreach (range(1,8) as $number) $valid = $valid && str_contains($authorization, sprintf('QMDB-P4-REQ-%03d', $number));
