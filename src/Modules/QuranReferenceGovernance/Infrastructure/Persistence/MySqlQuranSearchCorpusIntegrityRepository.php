...
final readonly class MySqlQuranSearchCorpusIntegrityRepository implements QuranSearchCorpusIntegrityRepository
{
    ...
    {
        $statement = $this->connections->connection()->prepare('SELECT BIN_TO_UUID(a.public_id) AS ayah_public_id, t.surah_number, t.ayah_number, t.global_ayah_ordinal, t.simple_clean_text, LOWER(HEX(t.text_sha256)) AS text_sha256, LOWER(HEX(a.text_sha256)) AS canonical_text_sha256 FROM quran_ayah_search_texts t INNER JOIN quran_ayahs a ON a.id = t.ayah_id AND a.release_id = t.canonical_release_id WHERE t.corpus_id = :corpus_id ORDER BY t.global_ayah_ordinal ASC, t.id ASC');
        $statement->execute([':corpus_id' => $corpusInternalId]);
        $rows = $statement->fetchAll(PDO::FETCH_ASSOC);
        if (!is_array($rows)) {
            throw new \RuntimeException('Qur’an corpus validation rows are invalid.');
        }
    ...
    {
        $statement = $this->connections->connection()->prepare('SELECT operation.request_fingerprint, BIN_TO_UUID(corpus.public_id) AS corpus_public_id, operation.expected_version FROM quran_search_corpus_validation_operations operation INNER JOIN quran_search_corpora corpus ON corpus.id = operation.corpus_id WHERE operation.submission_id = :submission_id FOR UPDATE');
        ...
        return ['corpus_public_id' => (string) $row['corpus_public_id'], 'expected_version' => (int) $row['expected_version']];
    }
    ...
    public function appendValidation(int $corpusInternalId, int $accountInternalId, string $evidenceSha256, DateTimeImmutable $occurredAt): string
    {
        ...
        $statement = $this->connections->connection()->prepare("INSERT INTO quran_search_corpus_validations (public_id,corpus_id,validator_code,validator_version,result,safe_summary_code,evidence_sha256,executed_by_type,executed_by_account_id,occurred_at,created_at) VALUES (:public_id,:corpus_id,'QMDB_QURAN_SEARCH_CORPUS_INTEGRITY','1','PASS','ALIGNED',:evidence,'ACCOUNT',:account_id,:occurred_at,:occurred_at)");
        $statement->execute([':public_id' => $publicId->toBinary(), ':corpus_id' => $corpusInternalId, ':evidence' => hex2bin($evidenceSha256), ':account_id' => $accountInternalId, ':occurred_at' => self::time($occurredAt)]);
