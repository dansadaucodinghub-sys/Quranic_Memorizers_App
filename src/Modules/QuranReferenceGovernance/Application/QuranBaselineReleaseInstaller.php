...
final readonly class QuranBaselineReleaseInstaller
{
    ...
    public function install(bool $dryRun): array
    {
        ...
        try {
            $exists = $pdo->query("SELECT id FROM quran_reference_releases WHERE release_code = 'QMDB_QURAN_TANZIL_UTHMANI_1_1' FOR UPDATE")->fetchColumn();
            if ($exists !== false) {
                throw new \DomainException('Baseline release already exists; use content verification rather than repair.');
            }
            if ($pdo->query("SELECT id FROM quran_reference_releases WHERE status = 'ACTIVE' FOR UPDATE")->fetchColumn() !== false) {
    ...
    /** @param list<array{surah_number:int,ayah_number:int,text:string,byte_size:int,sha256:string}> $text @param list<array<string,int|string>> $surahs */
    private function validate(array $text, array $surahs): void
    {
        ...
        foreach ($surahs as $surah) {
            if (($counts[(int) $surah['surah_number']] ?? 0) !== (int) $surah['ayah_count']) {
    ...
    /** @param list<array<string,int|string>> $surahs @return array<int,int> */
    private function insertSurahs(PDO $pdo, int $releaseId, int $artifactId, array $surahs): array
    {
        ...
        foreach ($surahs as $surah) {
            ...
            $ids[(int) $surah['surah_number']] = (int) $pdo->lastInsertId();
            $ordinal += $count;
        }
        return $ids;
    }

    /** @param array<int,int> $surahIds @param list<array{surah_number:int,ayah_number:int,text:string,byte_size:int,sha256:string}> $records */
