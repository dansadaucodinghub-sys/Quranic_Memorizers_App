...
final readonly class MediaRange
{
    ...
    public static function fromHeader(?string $header, int $length): ?self
    {
        ...
        if ($length < 1 || preg_match('/\Abytes=(\d*)-(\d*)\z/', $header, $parts) !== 1) {
            throw new \InvalidArgumentException('Range is invalid.');
        }
        $start = $parts[1] === '' ? null : (int) $parts[1];
        $end = $parts[2] === '' ? null : (int) $parts[2];
        ...
        if ($start === null) {
            if ($end < 1) {
                throw new \InvalidArgumentException('Range is invalid.');
            } $start = max(0, $length - $end);
            $end = $length - 1;
        } else {
            $end ??= $length - 1;
            if ($start >= $length || $end < $start) {
                throw new \InvalidArgumentException('Range is unsatisfiable.');
