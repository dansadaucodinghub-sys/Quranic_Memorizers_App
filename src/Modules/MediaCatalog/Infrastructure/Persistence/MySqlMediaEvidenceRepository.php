...
final readonly class MySqlMediaEvidenceRepository implements MediaEvidenceRepository
{
    ...
    public function lock(int $workspaceId, UuidV7 $assetId): ?array
    {
        ...
        foreach (['id','workspace_id','version','byte_size'] as $field) if (!is_int($row[$field]) && !ctype_digit((string) $row[$field])) throw new \UnexpectedValueException('Media asset row is malformed.');
        foreach (['public_id','status','original_storage_object_key'] as $field) if (!is_string($row[$field])) throw new \UnexpectedValueException('Media asset row is malformed.');
        if ($row['original_sha256'] !== null && !is_string($row['original_sha256'])) throw new \UnexpectedValueException('Media asset row is malformed.');
    ...
    {
        $id = UuidV7::generate(); $time = $now->setTimezone(new \DateTimeZone('UTC'))->format('Y-m-d H:i:s.u');
        $statement = $this->statement("INSERT INTO media_assets (public_id,workspace_id,created_by_account_id,purpose_code,media_kind,status,original_storage_provider_code,original_storage_object_key,original_filename_safe,byte_size,version,created_at,updated_at) VALUES (:public_id,:workspace,:actor,:purpose,:kind,'STAGING','LOCAL_PRIVATE',:storage_key,:filename,0,1,:now,:now)");
        $statement->execute([':public_id'=>$id->toBinary(),':workspace'=>$workspaceId,':actor'=>$actorAccountId,':purpose'=>$purpose,':kind'=>$kind,':storage_key'=>$storageKey,':filename'=>$safeFilename,':now'=>$time]);
        $locked = $this->lock($workspaceId, $id);
        if ($locked === null) throw new \RuntimeException('Staged media asset could not be locked.');
    ...
    {
        if (strlen($sha256) !== 32 || $byteSize < 1) throw new \InvalidArgumentException('Media integrity values are invalid.');
    ...
    {
        if (strlen($requestFingerprint) !== 32) throw new \InvalidArgumentException('Media submission fingerprint is invalid.');
        ...
        if($insert->rowCount()===1)return ['state'=>'CLAIMED','asset_id'=>null,'status'=>null,'version'=>null];
        ...
        $select->execute([':submission'=>$submission->toBinary()]); $row=$select->fetch(PDO::FETCH_ASSOC);
        if(!is_array($row)||!is_string($row['request_fingerprint']??null)||!is_string($row['operation_code']??null))throw new \UnexpectedValueException('Media submission receipt is malformed.');
        if((int)($row['workspace_id']??0)!==$workspaceId||(int)($row['actor_account_id']??0)!==$actorAccountId||$row['operation_code']!=='UPLOAD'||!hash_equals($row['request_fingerprint'],$requestFingerprint))return ['state'=>'CONFLICT','asset_id'=>null,'status'=>null,'version'=>null];
        if($row['completed_at']===null)throw new \DomainException('A matching media upload is already in progress.');
        if(!is_string($row['asset_public_id']??null)||!is_string($row['result_status']??null)||!is_numeric($row['result_version']??null))throw new \UnexpectedValueException('Completed media submission receipt is malformed.');
    ...
    public function completeUploadSubmission(UuidV7 $submission, string $assetPublicId, string $status, int $version, DateTimeImmutable $now): void
    {
        ...
        if($statement->rowCount()!==1)throw new \DomainException('Media submission completion changed concurrently.');
    }

    public function enqueueScan(int $workspaceId, int $assetId, DateTimeImmutable $now): void
    {
        $time=$now->setTimezone(new \DateTimeZone('UTC'))->format('Y-m-d H:i:s.u');
        $statement=$this->statement("INSERT IGNORE INTO media_processing_jobs (public_id,workspace_id,asset_id,job_type,status,attempt,max_attempts,available_at,version,created_at,updated_at) VALUES (:public_id,:workspace,:asset,'SCAN','QUEUED',0,3,:now,1,:now,:now)");
        $statement->execute([':public_id'=>UuidV7::generate()->toBinary(),':workspace'=>$workspaceId,':asset'=>$assetId,':now'=>$time]);
    }

    public function ensurePrivateDeliveryPolicy(int $workspaceId, int $assetId, DateTimeImmutable $now): void
    {
        $time=$now->setTimezone(new \DateTimeZone('UTC'))->format('Y-m-d H:i:s.u');
        $statement=$this->statement("INSERT IGNORE INTO media_delivery_policies (workspace_id,asset_id,visibility_code,rights_granted,consent_granted,version,created_at,updated_at) VALUES (:workspace,:asset,'PRIVATE',0,0,1,:now,:now)");
        $statement->execute([':workspace'=>$workspaceId,':asset'=>$assetId,':now'=>$time]);
    }

    public function findDeliverable(int $workspaceId, UuidV7 $assetId): ?array
    {
        $statement=$this->statement("SELECT a.original_storage_object_key,a.detected_mime_type,a.byte_size,a.original_sha256 FROM media_assets a INNER JOIN media_delivery_policies p ON p.asset_id=a.id AND p.workspace_id=a.workspace_id WHERE a.workspace_id=:workspace AND a.public_id=:public_id AND a.status IN ('APPROVED','PUBLISHED') AND p.rights_granted=1 AND p.consent_granted=1 AND NOT EXISTS (SELECT 1 FROM media_holds h WHERE h.asset_id=a.id AND h.released_at IS NULL) LIMIT 1");
        $statement->execute([':workspace'=>$workspaceId,':public_id'=>$assetId->toBinary()]); $row=$statement->fetch(PDO::FETCH_ASSOC);
        if(!is_array($row))return null;
        if(!is_string($row['original_storage_object_key']??null)||!is_string($row['detected_mime_type']??null)||!is_string($row['original_sha256']??null)||!is_numeric($row['byte_size']??null)||strlen($row['original_sha256'])!==32)throw new \UnexpectedValueException('Media delivery record is malformed.');
    ...
    private function statement(string $sql): PDOStatement
    {
        ...
        if (!$statement instanceof PDOStatement) throw new \RuntimeException('Media statement could not be prepared.');
        return $statement;
    }
}
