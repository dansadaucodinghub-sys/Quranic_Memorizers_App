...
{
    public function __construct(private string $command, private string $operation, private DatabaseConnectionProvider $connections, private ScheduledTaskMap $tasks, private MediaScanWorker $scans, private MediaProcessingWorker $processing, private EnvironmentVariables $environment, private MediaMaintenanceWorker $maintenance, private \Qmdb\Modules\MediaProcessing\Application\MediaRuntimeReadiness $runtimeReadiness)
    {
    }
    ...
    {
        return 'Run bounded P9 media ' . str_replace(':', ' ', $this->operation) . ' verification.';
    }
    ...
    {
        if (in_array($this->operation, ['uploads:expire', 'staging:cleanup', 'assets:reconcile', 'storage:reconcile'], true)) {
    ...
    {
        $this->requireTables();
        $scanner = $this->environment->optionalString('QMDB_MEDIA_SCANNER_BINARY');
        $ffmpeg = $this->environment->optionalString('QMDB_MEDIA_FFMPEG_BINARY');
        $ffprobe = $this->environment->optionalString('QMDB_MEDIA_FFPROBE_BINARY');
        $missing = [];
        ...
        $health = $this->runtimeReadiness->check();
        foreach ($health['checks'] as $name => $code) {
            $output->write($name . ': ' . $code . "\n");
        }
        $output->write('P9 production runtime readiness: ' . ($health['healthy'] ? 'PASS' : 'FAIL') . "\nScope: configured engines and schema only; not phase closeout.\n");
    ...
    private function smoke(ConsoleOutput $output): int
    {
        ...
        foreach (['media.scans.process', 'media.processing.process', 'media.staging.cleanup', 'media.uploads.expire', 'media.assets.reconcile'] as $task) {
            if (!in_array($task, $taskIds, true)) {
                throw new \RuntimeException('Required P9 scheduler task is missing.');
            }
        }
    ...
    private function requireTables(): void
    {
        ...
        if (!$statement instanceof \PDOStatement) {
            throw new \RuntimeException('P9 table check could not be prepared.');
        }
