...
final readonly class DeviceRevocationController implements Controller
{
    public function __construct(
        private AuthenticatedRequestGuard $guard,
        private AccountSessionInventoryHandler $inventory,
        private RemoteDeviceRevocationService $revocation,
    ...
    public function handle(ServerRequestInterface $request): ResponseInterface
    {
        ...
        try {
            $target = DeviceId::fromString($this->parameter($request, 'deviceId'));
        } catch (\InvalidArgumentException) {
            return $this->responses->createResponse(404);
        }
        $csrf = $this->csrf->issue($request, CsrfAction::ACCOUNT_DEVICE_REVOKE);
        if (strtoupper($request->getMethod()) === 'GET') {
            foreach ($this->inventory->handle($context)->devices as $device) {
                if ($device->publicId === $target->toString()) {
                    if ($device->publicId === $context->deviceId->toString()) {
                        return $this->responses->createResponse(409);
                    }

                    return $this->confirmation($request, $target, $device->version, $csrf);
                }
            }
        ...
        if (!$this->csrf->validates($request, CsrfAction::ACCOUNT_DEVICE_REVOKE, $csrf['cookie'], $token)) {
    ...
    private function confirmation(
        ...
        return $this->view->render(
            ...
            IdentitySessionViewDataFactory::confirmation('device', $target->toString(), $version, $csrf['token']),
            'title.device_revoke',
            cookie: $csrf['cookie'],
