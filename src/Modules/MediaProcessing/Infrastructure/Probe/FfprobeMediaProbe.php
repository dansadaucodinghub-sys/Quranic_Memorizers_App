...
final readonly class FfprobeMediaProbe implements MediaProbe
{
    public function __construct(private MediaBlobStore $storage, private string $binary, private int $timeoutSeconds = 30)
    {
        if ($binary === '' || str_contains($binary, "\0") || $timeoutSeconds < 1 || $timeoutSeconds > 300) {
            throw new \InvalidArgumentException('FFprobe configuration is invalid.');
        }
    }
    public function inspect(string $privateObjectKey): array
    {
        ...
        if ($file === false || file_put_contents($file, $contents, LOCK_EX) !== strlen($contents)) {
            throw new \RuntimeException('Media probe temporary storage is unavailable.');
        }
        try {
            $output = $this->run([$this->binary, '-v', 'error', '-protocol_whitelist', 'file,pipe', '-show_entries', 'format=duration,format_name:stream=codec_name,codec_type,width,height', '-of', 'json', $file]);
            $decoded = json_decode($output, true, 16, JSON_THROW_ON_ERROR);
            if (!is_array($decoded)) {
                throw new \RuntimeException('Media probe returned invalid metadata.');
            }
    ...
    private function run(array $command): string
    {
        if (!is_file($this->binary)) {
            throw new \RuntimeException('Trusted FFprobe executable is unavailable.');
        }
        $result = \Qmdb\Modules\MediaProcessing\Infrastructure\Process\BoundedMediaProcess::run($command, $this->timeoutSeconds);
        if ($result['exit'] !== 0) {
            throw new \RuntimeException('Trusted media probe failed.');
        }
