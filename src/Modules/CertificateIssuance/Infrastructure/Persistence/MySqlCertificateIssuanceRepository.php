...
final readonly class MySqlCertificateIssuanceRepository implements CertificateIssuanceRepository
{
    ...
    public function lockActiveSigningKey(): ?array
    {
        ...
        $statement->execute(); $row = self::row($statement->fetch(PDO::FETCH_ASSOC));
    ...
    public function completed(UuidV7 $submissionId, string $fingerprint): ?array
    {
        ...
        $statement->execute([':submission_id' => $submissionId->toBinary()]); $row = self::row($statement->fetch(PDO::FETCH_ASSOC));
        if ($row === null) return null;
        if (!hash_equals(PdoResultReader::string($row, 'request_fingerprint'), $fingerprint)) throw new \DomainException('Certificate submission conflicts with a prior request.');
        $certificate = $this->statement('SELECT public_id FROM certificates WHERE id=:id'); $certificate->execute([':id' => PdoResultReader::integer($row, 'certificate_id')]); $public = $certificate->fetchColumn();
        if (!is_string($public)) throw new \RuntimeException('Certificate replay target is unavailable.');
    ...
    {
        $id = UuidV7::generate(); $time = self::time($now);
    ...
    public function issue(array $certificate, string $manifest, string $manifestHash, string $signature, string $pdfHash, int $actorAccountId, DateTimeImmutable $now): bool
    {
        ...
        $statement->execute([':manifest' => $manifest, ':manifest_hash' => $manifestHash, ':signature' => $signature, ':pdf_hash' => $pdfHash, ':actor' => $actorAccountId, ':now' => self::time($now), ':id' => $certificate['id'], ':workspace_id' => $certificate['workspace_id'], ':version' => $certificate['version']]); return $statement->rowCount() === 1;
    }

    /** @param CertificateRecord $certificate */
    public function transition(array $certificate, string $target, int $actorAccountId, ?string $reason, DateTimeImmutable $now): bool
    {
        $extra = match ($target) { 'REVOKED' => 'revoked_by_account_id=:actor,revoked_at=:now,revocation_reason_code=:reason', 'ARCHIVED' => 'archived_at=:now', default => throw new \InvalidArgumentException('Certificate target is invalid.') };
        ...
        $statement->execute([':status' => $target, ':actor' => $actorAccountId, ':reason' => $reason, ':now' => self::time($now), ':id' => $certificate['id'], ':workspace_id' => $certificate['workspace_id'], ':previous' => $certificate['status'], ':version' => $certificate['version']]); return $statement->rowCount() === 1;
    }
    ...
    {
        foreach ([['PDF',$pdfKey,'application/pdf',$pdf], ['MANIFEST',$manifestKey,'application/json',$manifest]] as [$type,$key,$media,$bytes]) { $statement = $this->statement('INSERT INTO certificate_artifacts (public_id,workspace_id,certificate_id,artifact_type,storage_provider_code,storage_object_key,media_type,byte_size,sha256,renderer_code,renderer_version,created_at) VALUES (:public_id,:workspace_id,:certificate_id,:type,\'LOCAL_PRIVATE\',:object_key,:media_type,:byte_size,:sha256,\'dompdf\',\'3.1\',:now)'); $statement->execute([':public_id'=>UuidV7::generate()->toBinary(), ':workspace_id'=>$certificate['workspace_id'], ':certificate_id'=>$certificate['id'], ':type'=>$type, ':object_key'=>$key, ':media_type'=>$media, ':byte_size'=>strlen($bytes), ':sha256'=>hash('sha256',$bytes,true), ':now'=>self::time($now)]); }
    }
    ...
    private function statement(string $sql): PDOStatement { $statement=$this->connections->connection()->prepare($sql); if (!$statement instanceof PDOStatement) throw new \RuntimeException('Certificate statement could not be prepared.'); return $statement; }
    /** @return array<string,mixed>|null */
    private static function row(mixed $value): ?array
    {
        if (!is_array($value)) return null;
        foreach ($value as $key => $_) if (!is_string($key)) throw new \UnexpectedValueException('Certificate database row is malformed.');
        return $value;
    }
    /**
