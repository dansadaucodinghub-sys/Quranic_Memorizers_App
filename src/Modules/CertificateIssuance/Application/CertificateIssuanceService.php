...
final readonly class CertificateIssuanceService
{
    ...
    public function prepare(AuthenticatedAccountContext $actor, AccountWorkspaceTenantContext $tenant, UuidV7 $submission, UuidV7 $publicationId, UuidV7 $rowId, UuidV7 $templateId, string $type): array
    {
        ...
        return $this->transactions->transactional(function () use ($actor,$tenant,$submission,$publicationId,$rowId,$templateId,$type): array {
            ...
            $completed=$this->repository->completed($submission,$fingerprint); if ($completed !== null) return ['certificate_id'=>$completed['certificate_id'],'certificate_number'=>'REPLAYED','verification_code'=>'','status'=>$completed['status'],'version'=>$completed['version'],'replayed'=>true];
            $source=$this->repository->lockFinalizedResultRow($tenant->workspaceInternalId,$publicationId,$rowId); if ($source===null) throw new \DomainException('A FINALIZED publication result row is required.');
            $template=$this->repository->lockActiveTemplate($tenant->workspaceInternalId,$templateId); if ($template===null) throw new \DomainException('An ACTIVE certificate template is required.');
            $key=$this->repository->lockActiveSigningKey(); if ($key===null) throw new \DomainException('An ACTIVE certificate signing key is required.');
            $verification=CertificateVerificationCode::generate(); $now=$this->clock->now();
    ...
    public function issue(AuthenticatedAccountContext $actor, AccountWorkspaceTenantContext $tenant, UuidV7 $submission, UuidV7 $certificateId, int $expectedVersion, string $verificationCode, string $verificationUrl): array
    {
        ...
        return $this->transactions->transactional(function () use ($actor,$tenant,$submission,$certificateId,$expectedVersion,$verificationCode,$verificationUrl): array {
            ...
            $this->lifecycle->assertTransition($certificate['status'],'ISSUED'); $this->stepUp->consumeWithGrant($actor,StepUpAction::CERTIFICATE_ISSUE);
            $key=new CertificateSigningKey($certificate['key_code'],$certificate['provider_code'],$certificate['provider_key_reference'],$certificate['public_key'],$certificate['key_status']); if(!$key->canIssue() || !$this->keys->verifiesPublicKey($key)) throw new \DomainException('Certificate signing key is unavailable.');
            $now=$this->clock->now();
            ...
            $signature=$this->keys->sign($key,$manifest); $rendered=$this->renderer->render($verificationUrl,['display_name'=>$certificate['display_name'],'certificate_number'=>$certificate['certificate_number'],'certificate_type'=>$certificate['certificate_type'],'issued_at'=>$now->format('Y-m-d')]);
            $pdfKey='certificates/' . $certificate['public_id'] . '/certificate.pdf'; $manifestKey='certificates/' . $certificate['public_id'] . '/manifest.json'; $this->artifacts->put($pdfKey,$rendered->pdf,'application/pdf'); $this->artifacts->put($manifestKey,$manifest,'application/json');
            $manifestHash=hash('sha256',$manifest,true); if(!$this->repository->issue($certificate,$manifest,$manifestHash,$signature,$rendered->pdfSha256(),$actor->accountInternalId,$now)) throw new \DomainException('Certificate changed concurrently.');
            $this->repository->appendArtifacts($certificate,$pdfKey,$rendered->pdf,$manifestKey,$manifest,$now); $this->repository->appendEvent($certificate,'ISSUED','ISSUED',$actor->accountInternalId,null,$now); $this->repository->record($submission,$fingerprint,'CERTIFICATE_ISSUE',$certificate,'ISSUED',$certificate['version']+1,$now);
    ...
    {
        $rule=match($operation){'CERTIFICATE_REVOKE'=>['REVOKED','REVOKED','workspace.certificates.revoke',StepUpAction::CERTIFICATE_REVOKE,SecurityEventCode::CERTIFICATE_REVOKED], 'CERTIFICATE_ARCHIVE'=>['ARCHIVED','ARCHIVED','workspace.certificates.archive',StepUpAction::CERTIFICATE_ARCHIVE,SecurityEventCode::CERTIFICATE_REVOKED], default=>throw new \InvalidArgumentException('Certificate transition operation is invalid.')};
        if($rule[0]==='REVOKED' && (preg_match('/\A[A-Z][A-Z0-9_]{1,62}\z/',$reason??'')!==1)) throw new \InvalidArgumentException('Certificate revocation requires a safe reason code.');
        $this->allow($actor,$tenant,$rule[2]); $this->rate($actor);
        return $this->transactions->transactional(function()use($actor,$tenant,$submission,$certificateId,$expectedVersion,$operation,$reason,$rule):array{$fingerprint=$this->fingerprint($tenant,$actor,$operation,$certificateId->toString(),$expectedVersion);$completed=$this->repository->completed($submission,$fingerprint);if($completed!==null)return ['certificate_id'=>$completed['certificate_id'],'status'=>$completed['status'],'version'=>$completed['version'],'replayed'=>true];$certificate=$this->repository->lockCertificate($tenant->workspaceInternalId,$certificateId);if($certificate===null||$certificate['version']!==$expectedVersion)throw new \DomainException('Certificate transition is stale or unavailable.');$this->lifecycle->assertTransition($certificate['status'],$rule[0]);$this->stepUp->consumeWithGrant($actor,$rule[3]);$now=$this->clock->now();if(!$this->repository->transition($certificate,$rule[0],$actor->accountInternalId,$reason,$now))throw new \DomainException('Certificate changed concurrently.');$this->repository->appendEvent($certificate,$rule[1],$rule[0],$actor->accountInternalId,$reason,$now);$this->repository->record($submission,$fingerprint,$operation,$certificate,$rule[0],$certificate['version']+1,$now);$this->audit->workspace($rule[4],$tenant->workspacePublicId(),SecurityEventSubjectKind::CERTIFICATE,$certificate['public_id'],$actor->accountId->toString(),$now,['certificate_number'=>$certificate['certificate_number'],'previous_status'=>$certificate['status'],'new_status'=>$rule[0],'version'=>$certificate['version']+1],$reason);return ['certificate_id'=>$certificate['public_id'],'status'=>$rule[0],'version'=>$certificate['version']+1,'replayed'=>false];},TransactionOptions::readWrite(retryPolicy:new TransactionRetryPolicy(3,15,150)));
    }

    private function allow(AuthenticatedAccountContext $actor,AccountWorkspaceTenantContext $tenant,string $permission):void{$this->authorization->requireAllowed(new AuthorizationRequest(AuthorizationSubject::fromAuthenticatedContext($actor),new PermissionCode($permission),new WorkspaceAuthorizationScope($tenant)));}
    private function rate(AuthenticatedAccountContext $actor):void{$attempt=new IdentityRateLimitAttempt(IdentityRateLimitScope::CERTIFICATE_ISSUE_ACCOUNT,$this->fingerprints->generate('certificate-issuance-account',(string)$actor->accountInternalId),new IdentityRateLimitPolicy(60,10,60));if(!$this->rateLimits->consume([$attempt],$this->clock->now())->allowed)throw new \DomainException('Certificate operation is temporarily unavailable.');}
    private function fingerprint(AccountWorkspaceTenantContext $tenant,AuthenticatedAccountContext $actor,string $operation,string $aggregate,int $version):string{return $this->fingerprints->generate('certificate-operation',implode("\0",[$tenant->workspaceInternalId,$actor->accountInternalId,$operation,$aggregate,$version]))->toBinary();}
}
