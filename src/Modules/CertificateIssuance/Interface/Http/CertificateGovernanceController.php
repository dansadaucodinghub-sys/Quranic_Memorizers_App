...
final readonly class CertificateGovernanceController implements Controller
{
    ...
    public function handle(ServerRequestInterface $request): ResponseInterface
    {
        ...
        $route=$request->getAttribute(RouteAttributes::NAME); if(!is_string($route))return $this->response(404,'Certificate route is unavailable.');
        try {$tenant=$this->tenant->require($request,true);} catch(TenantContextRequiredException){return $this->responses->createResponse(303)->withHeader('Location','/account/workspaces?context_required=1')->withHeader('Cache-Control','private, no-store');}
        $action=$this->csrfAction($route); $csrf=$this->csrf->issue($request,$action);
        if(strtoupper($request->getMethod())==='GET')return $this->form($route,$request,$csrf['token'],$csrf['cookie']->setCookieHeader);
        $body=$request->getParsedBody(); if(!is_array($body)||!is_string($body['csrf_token']??null)||!$this->csrf->validates($request,$action,$csrf['cookie'],$body['csrf_token']))return $this->response(403,'Request verification failed.',$csrf['cookie']->setCookieHeader);
        try {
            ...
            if($route==='workspace.certificates.issue') {
                ...
                $parameters=$request->getAttribute(RouteAttributes::PARAMETERS); if(!is_array($parameters)||!is_string($parameters['certificateId']??null))throw new \InvalidArgumentException('Certificate identifier is invalid.');
        ...
        } catch(\DomainException $error){return $this->response(str_contains($error->getMessage(),'temporarily')?429:409,'Certificate action could not be completed.',$csrf['cookie']->setCookieHeader);} catch(\InvalidArgumentException){return $this->response(422,'Certificate request is invalid.',$csrf['cookie']->setCookieHeader);}
    ...
    {
        $path=$request->getUri()->getPath(); $issue=$route==='workspace.certificates.issue.form';
        ...
        if($issue)$path='/workspace/certificates/issue'; else $path=rtrim($path,'/');
    ...
    private function csrfAction(string $route):CsrfAction{return match($route){'workspace.certificates.issue.form','workspace.certificates.issue'=>CsrfAction::CERTIFICATE_ISSUE,'workspace.certificates.revoke.form','workspace.certificates.revoke'=>CsrfAction::CERTIFICATE_REVOKE,'workspace.certificates.archive.form','workspace.certificates.archive'=>CsrfAction::CERTIFICATE_ARCHIVE,default=>throw new \InvalidArgumentException('Certificate CSRF route is invalid.')};}
    /** @param array<array-key,mixed> $body */ private function field(array $body,string $name,int $limit):string{$value=$body[$name]??null;if(!is_string($value)||$value===''||strlen($value)>$limit)throw new \InvalidArgumentException('Certificate field is invalid.');return $value;}
    /** @param array<array-key,mixed> $body */ private function integer(array $body,string $name):int{$value=$this->field($body,$name,10);if(preg_match('/\A[1-9][0-9]*\z/',$value)!==1)throw new \InvalidArgumentException('Certificate version is invalid.');return(int)$value;}
    ...
    private function withCookie(ResponseInterface $response,?string $cookie):ResponseInterface{return $cookie===null?$response:$response->withAddedHeader('Set-Cookie',$cookie);}
    private function safePath(ServerRequestInterface $request):string{$path=$request->getUri()->getPath();return str_starts_with($path,'/workspace/')?$path:'/workspace/certificates/issue';}
