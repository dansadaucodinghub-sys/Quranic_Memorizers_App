...
final readonly class CompetitionP6VerifyConsoleCommand implements ConsoleCommand
{
    ...
    {
        return new ConsoleCommandName('competition:p6:verify');
    }
    public function description(): string
    {
        return 'Verify P6 judging, fixed-point scoring, results, appeals, and P7 boundary.';
    }
    public function execute(ConsoleInput $input, ConsoleOutput $output): int
    {
        ...
        try {
            ...
            if ($statement === false) {
                throw new \RuntimeException('P6 schema verification statement could not be prepared.');
            }
            foreach ($required as $table) {
                $statement->execute([':table' => $table]);
                if ((int) $statement->fetchColumn() !== 1) {
                    throw new \RuntimeException("Required P6 table is missing: {$table}");
                }
            }
