...
{
    public function id(): MigrationId { return new MigrationId('20260911139900_add_competition_roster_entry_tenant_key'); }
    public function description(): string { return 'Add a roster-entry tenant candidate key for P6 participant isolation.'; }
    public function dependencies(): array { return [(new HardenCompetitionTenantRelationshipsMigration())->id()]; }
    public function up(): array { return [new SqlMigrationStep(new MigrationStepId('001_roster_entry_tenant_key'), 'Add roster entry workspace candidate key.', 'ALTER TABLE competition_roster_entries ADD UNIQUE KEY uq_competition_roster_entries_workspace_id (workspace_id,id)')]; }
