...
final readonly class AuthorizedMediaUploadService
{
    ...
    public function upload(AuthenticatedAccountContext $actor, AccountWorkspaceTenantContext $tenant, UuidV7 $submission, string $purpose, string $kind, string $filename, string $contents): array
    {
        ...
        if(!$this->rateLimits->consume([$attempt],$this->clock->now())->allowed)throw new \DomainException('Media upload is temporarily unavailable.');
        if($contents==='')throw new \InvalidArgumentException('Media upload is empty.');
        return $this->transactions->transactional(function()use($actor,$tenant,$submission,$purpose,$kind,$filename,$contents):array{
            ...
            if($receipt['state']==='CONFLICT')throw new \DomainException('Media idempotency submission conflicts with a different request.');
            if($receipt['state']==='REPLAY')return ['asset_id'=>$receipt['asset_id'],'status'=>$receipt['status'],'version'=>$receipt['version'],'replayed'=>true];
            $now=$this->clock->now(); $asset=$this->workflow->initiate($tenant->workspaceInternalId,$actor->accountInternalId,$purpose,$kind,$filename,'quarantine/'.$submission->toString().'.bin',$now);
            ...
            $this->assets->ensurePrivateDeliveryPolicy($tenant->workspaceInternalId,$asset['id'],$now); $this->assets->enqueueScan($tenant->workspaceInternalId,$asset['id'],$now);
            $this->assets->completeUploadSubmission($submission,$result['asset_id'],$result['status'],$result['version'],$now);
            $this->audit->workspace(SecurityEventCode::MEDIA_ASSET_UPLOADED,$tenant->workspacePublicId(),SecurityEventSubjectKind::MEDIA_ASSET,$result['asset_id'],$actor->accountId->toString(),$now,['purpose_code'=>$purpose,'media_kind'=>$kind,'byte_size'=>strlen($contents),'sha256'=>$result['sha256'],'status'=>$result['status'],'version'=>$result['version']]);
