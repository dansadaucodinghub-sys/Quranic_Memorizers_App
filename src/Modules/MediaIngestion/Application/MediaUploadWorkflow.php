...
final readonly class MediaUploadWorkflow
{
    ...
    public function initiate(int $workspaceId, int $actorAccountId, string $purpose, string $mediaKind, string $displayFilename, string $opaqueStorageKey, DateTimeImmutable $now): array
    {
        ...
        if (preg_match('/\A[A-Z][A-Z0-9_]{1,47}\z/', $purpose) !== 1 || !in_array($mediaKind, ['AUDIO','VIDEO','IMAGE'], true)) throw new \InvalidArgumentException('Media upload purpose or kind is invalid.');
    ...
    {
        $this->assertIdentity($workspaceId, $actorAccountId);
        if ($expectedVersion < 1 || $contents === '' || strlen($contents) > MediaUploadPolicy::MAX_UPLOAD_BYTES) throw new \InvalidArgumentException('Media upload finalization input is invalid.');
        $inspection = MediaUploadPolicy::inspect($contents);
        $asset = $this->assets->lock($workspaceId, $assetId);
        if ($asset === null || $asset['version'] !== $expectedVersion) throw new \DomainException('Media upload is stale or unavailable.');
        if ($asset['status'] === 'QUARANTINED' && $asset['sha256'] !== '' && hash_equals($asset['sha256'], hash('sha256', $contents, true))) return ['asset_id'=>$asset['public_id'],'status'=>'QUARANTINED','version'=>$asset['version'],'mime'=>$inspection['mime'],'sha256'=>hash('sha256',$contents)];
        ...
        if (!$this->assets->finalizeUpload($asset, $inspection['mime'], strlen($contents), $hash, $now)) throw new \DomainException('Media upload changed concurrently.');
    ...
    private function assertIdentity(int $workspaceId, int $actorAccountId): void { if ($workspaceId < 1 || $actorAccountId < 1) throw new \InvalidArgumentException('Media upload actor context is invalid.'); }
    private function safeFilename(string $filename): string
    {
        if ($filename === '' || strlen($filename) > 180 || str_contains($filename, "\0") || !preg_match('//u', $filename)) throw new \InvalidArgumentException('Media filename is invalid.');
        $safe = trim(preg_replace('/[\\\\\/\x00-\x1F\x7F]+/u', '_', $filename) ?? '');
        if ($safe === '' || $safe === '.' || $safe === '..') throw new \InvalidArgumentException('Media filename is invalid.');
        return $safe;
    }
}
