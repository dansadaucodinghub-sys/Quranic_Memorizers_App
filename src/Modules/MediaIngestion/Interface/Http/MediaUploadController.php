...
{
    public function __construct(private AuthenticatedRequestGuard $authentication, private TenantContextRequiredGuard $tenant, private IdentityCsrf $csrf, private AuthorizedMediaUploadService $uploads, private Psr17Factory $responses, private \Qmdb\Modules\SecurityAuthorization\Application\AuthorizationRequirementGuard $authorization)
    {
    }
    ...
    public function handle(ServerRequestInterface $request): ResponseInterface
    {
        ...
        try {
            ...
            $this->authorization->requireAllowed(new \Qmdb\Modules\SecurityAuthorization\Application\AuthorizationRequest(\Qmdb\Modules\SecurityAuthorization\Application\AuthorizationSubject::fromAuthenticatedContext($actor), new \Qmdb\Modules\SecurityAuthorization\Domain\PermissionCode('media.assets.upload'), new \Qmdb\Modules\SecurityAuthorization\Domain\WorkspaceAuthorizationScope($tenant)));
        } catch (TenantContextRequiredException) {
            return $this->redirect('/account/workspaces?context_required=1');
        } catch (AuthorizationDeniedException) {
            return $this->response(403, 'You are not permitted to upload media.');
        }
        $csrf = $this->csrf->issue($request, CsrfAction::MEDIA_UPLOAD);
        if (strtoupper($request->getMethod()) === 'GET') {
            return $this->form($csrf['token'], $csrf['cookie']->setCookieHeader, $tenant->workspacePublicId(), (string)$tenant->version->value);
        }
        $body = $request->getParsedBody();
        if (!is_array($body) || !is_string($body['csrf_token'] ?? null) || !$this->csrf->validates($request, CsrfAction::MEDIA_UPLOAD, $csrf['cookie'], $body['csrf_token'])) {
            return $this->response(403, 'Request verification failed.', $csrf['cookie']->setCookieHeader);
        }
        try {
            if ($this->field($body, 'workspace_id', 36) !== $tenant->workspacePublicId() || $this->field($body, 'tenant_context_version', 20) !== (string)$tenant->version->value) {
                throw new \DomainException('Workspace selection changed.');
            }
            if ($request->hasHeader('X-QMDB-CSRF') && !hash_equals($body['csrf_token'], $request->getHeaderLine('X-QMDB-CSRF'))) {
            ...
            if ($request->hasHeader('Idempotency-Key') && !hash_equals($submission->toString(), $request->getHeaderLine('Idempotency-Key'))) {
                throw new \InvalidArgumentException('Conflicting request tokens.');
            }
            $purpose = $this->field($body, 'purpose_code', 48);
            $kind = $this->field($body, 'media_kind', 16);
            $upload = $request->getUploadedFiles()['media_file'] ?? null;
            if (!$upload instanceof UploadedFileInterface || $upload->getError() !== UPLOAD_ERR_OK) {
                throw new \InvalidArgumentException('Media file upload is unavailable.');
            }
            $name = $upload->getClientFilename();
            if (!is_string($name)) {
                throw new \InvalidArgumentException('Media filename is unavailable.');
            }
            $contents = $this->boundedContents($upload);
            $result = $this->uploads->upload($actor, $tenant, $submission, $purpose, $kind, $name, $contents);
        } catch (AuthorizationDeniedException) {
            return $this->response(403, 'You are not permitted to upload media.', $csrf['cookie']->setCookieHeader);
        } catch (\InvalidArgumentException) {
            return $this->response(422, 'Media upload request is invalid.', $csrf['cookie']->setCookieHeader);
        } catch (\DomainException $error) {
            return $this->response(str_contains($error->getMessage(), 'temporarily') ? 429 : 409, 'Media upload could not be completed.', $csrf['cookie']->setCookieHeader);
        }
        return $this->redirect('/workspace/media/uploads?asset=' . rawurlencode($result['asset_id']), $csrf['cookie']->setCookieHeader);
    }

    private function form(string $token, ?string $cookie, string $workspace, string $version): ResponseInterface
    {
        ...
        $html = str_replace('<input type="hidden" name="csrf_token"', '<input type="hidden" name="workspace_id" value="' . $this->escape($workspace) . '"><input type="hidden" name="tenant_context_version" value="' . $this->escape($version) . '"><input type="hidden" name="csrf_token"', $html);
    ...
    /** @param array<array-key,mixed> $body */ private function field(array $body, string $name, int $limit): string
    {
        ...
        if (!is_string($value) || $value === '' || strlen($value) > $limit) {
            throw new \InvalidArgumentException('Media field is invalid.');
        }return $value;
    }
    private function boundedContents(UploadedFileInterface $upload): string
    {
        $stream = $upload->getStream();
        $contents = '';
        $limit = min(MediaUploadPolicy::MAX_UPLOAD_BYTES, 16_777_216);
        while (!$stream->eof()) {
            ...
            if (strlen($contents) > $limit) {
                throw new \InvalidArgumentException('Media file exceeds the direct-upload limit. Use the resumable upload workflow.');
            }
        } return $contents;
    }
    ...
    private function redirect(string $location, ?string $cookie = null): ResponseInterface
    {
        $response = $this->responses->createResponse(303)->withHeader('Location', $location)->withHeader('Cache-Control', 'private, no-store');
