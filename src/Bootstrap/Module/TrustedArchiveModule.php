...
final readonly class TrustedArchiveModule implements Module
{
    ...
    {
        return new ModuleId('trusted.archive');
    }
    public function dependencies(): array
    {
        return [new ModuleId('certificate.issuance'), new ModuleId('record.passport'), new ModuleId('competition.result_publication'), new ModuleId('foundation.database'), new ModuleId('security.audit'), new ModuleId('security.authorization'), new ModuleId('tenancy.context')];
    }
    public function register(ModuleRegistrationContext $context): void
    {
        ...
        foreach ([['record_passports.project','Project issued certificates into private Person passports.',60],['trusted_archive.seal_pending','Seal bounded issued-certificate archive records.',60],['trusted_archive.reconcile','Verify bounded trusted-archive hash chains.',900]] as [$id,$description,$interval]) {
            $context->scheduledTask(new ScheduledTaskRegistration(new ScheduledTaskId($id), $description, new FixedIntervalSchedule($interval), P8MaintenanceTask::class, 120, 'trusted.archive'));
        }
    }
}
